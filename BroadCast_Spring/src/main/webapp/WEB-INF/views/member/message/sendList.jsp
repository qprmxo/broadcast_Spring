...
<script type="text/javascript">
	...
	<button type="button" id="btnDelete" class="w3-button w3-teal w3-round-large" style="margin-left: 505px;">削除</button>
	
	<br>
	
	<form action="msgSendDelete" method="post" id="frm">
		...
		<table border="1" style="margin:auto;">
			...
			<c:forEach var="messageVo" items="${messageList }">
				<tr>
					...
					<td>${messageVo.rid }</td>
					<td><a href="msgInfo?msg_num=${messageVo.msg_num }&cmd=" 
	...
	<div class="w3-bar-all w3-center">
		<c:choose>
			<c:when test="${pu.startPageNum>5 }">
				<a href="sendList?pageNum=${pu.startPageNum-1 }" class="w3-bar-item w3-button w3-hover-black">前へ</a>
			</c:when>
			<c:otherwise>
			...
			<c:otherwise>
				<c:forEach var="i" begin="${pu.startPageNum }" end="${pu.endPageNum }">
					<c:choose>
						...
						<c:otherwise>
							<a href="sendList?pageNum=${i }" class="w3-bar-item w3-button w3-hover-black">${i }</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</c:otherwise>
		...
		<c:choose>
			<c:when test="${pu.endPageNum<pu.totalPageCount }">
				<a href="sendList?pageNum=${pu.endPageNum+1 }'/>" class="w3-bar-item w3-button w3-hover-black">次へ</a>
			</c:when>
			<c:otherwise>
